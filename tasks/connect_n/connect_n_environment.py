...
class ConnectN:
    ...
    def __init__(self, size, N, pie_rule=False):
        ...
        if self.w < 0 or self.h < 0 or self.N < 2 or \
            ...
            raise ValueError(
                'Game cannot initialize with a {0:d}x{1:d} grid, and winning condition {2:d} in a row'.format(self.w,
                                                                                                              self.h,
                                                                                                              self.N))
...
def train(mcts_tree, agent, episodes: int = 400, num_exploration_steps: int = 50):
    outcomes = []
    losses = []
    ...
    for i_episode, e in enumerate(range(episodes), start=1):

        # # mytree = mcts.Node(ConnectN(**game_setting))
        # vterm = []
        # logterm = []

        while mcts_tree.outcome is None:
            for _ in range(num_exploration_steps):
                mcts_tree.explore(agent.policy)

            current_player = mcts_tree.game.player
            mcts_tree, (v, nn_v, p, nn_p) = mcts_tree.next()
            mcts_tree.detach_mother()

            agent.step(v, nn_v, p, nn_p)
